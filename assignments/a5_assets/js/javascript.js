...
function hideGame2()
{
	...
	document.getElementById("gameDiv2").disabled = true;
}


function ajax(winner)
{
	$.ajax({ url: 'rps.php',
	         data: {action: winner},
			 type: 'post',
			 ...
			 success: function(output)
			 {
					...
					//Parse data
					var numberWins	= output.wins;
					var numberLosses = parseInt(output.losses);
					var numberTies	= parseInt(output.ties);
					
					
										
					//Print results
					$("#results").html('<p id="game_results">'
						+ '<br /> User Wins: ' + numberWins
						+ '<br /> Computer Wins: ' + numberLosses
						+ '<br /> Tied Games: ' + numberTies
						+ '</p>');
